...
function ProgressiveSlotsGame() {
  const { user, fetchUser } = useAuth();
  const { showError, showInsufficientBalance } = useNotification();
  const navigate = useNavigate();
  const [settings, updateSettings] = useGameSettings('progressiveslots', {
    bet: 5,
    rulesTab: 'howtoplay'
  });
  ...
  const performSpin = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_URL}/games/progressiveslots/play`,
        { bet },
          ...
          <div className="rules-section">
            ...
            <div className="rules-content">
              <div className="rules-objective">
                <p><strong>Objective:</strong> Spin the reels and match symbols to win from the progressive jackpot. Three matching symbols win!</p>
              </div>
                ...
                {rulesTab === 'howtoplay' && (
                  <div className="tab-panel">
                    <ul>
                      ...
                      <li>Higher bets increase jackpot contribution</li>
                    </ul>
                  </div>
                )}
...
export default ProgressiveSlotsGame;
