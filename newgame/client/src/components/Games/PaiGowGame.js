...
import './PaiGowGame.css';

function PaiGowGame() {
  const { user, fetchUser } = useAuth();
  const { showError, showInsufficientBalance } = useNotification();
  const navigate = useNavigate();
  const [settings, updateSettings] = useGameSettings('paigow', {
    bet: 10,
    rulesTab: 'howtoplay'
  });
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/games/paigow/play`,
        { bet },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // Generate random cards for display (7 cards for Pai Gow)
      const suits = ['♠', '♥', '♦', '♣'];
  ...
  // Split cards into high hand (5 cards) and low hand (2 cards)
  const highHand = cards.slice(0, 5);
  const lowHand = cards.slice(5, 7);
                ...
                {cards.length > 0 ? (
                  <div className="cards-display">
                    <div className="player-cards">
                      <h3>Your Hand (7 Cards)</h3>
                      
                      {/* High Hand - 5 cards */}
                      <div className="hand-section">
                        <h4>High Hand (5 cards)</h4>
                        <div className="cards-row">
                          {highHand.map((card, index) => {
                      ...
                      {/* Low Hand - 2 cards */}
                      <div className="hand-section">
                        <h4>Low Hand (2 cards)</h4>
                        <div className="cards-row">
                          {lowHand.map((card, index) => {
                            const display = getCardDisplay(card);
                            if (!display) return null;
                            const isAnimating = animatingCards.includes(index + 5);
                            return (
                              <div key={index + 5} className={`card ${isAnimating ? 'dealing' : ''} ${display.color}`}>
                                <div className="card-inner">
                                  <div className="card-corner card-top">
                                    <div className="card-rank">{display.rank}</div>
                                    <div className="card-suit">{display.suit}</div>
              ...
              <div className="game-rules">
                ...
                <div className="rules-content">
                  <div className="rules-objective">
                    <p><strong>Objective:</strong> Create two poker hands from seven cards - a high hand (5 cards) and a low hand (2 cards).</p>
                  </div>
                    ...
                    {rulesTab === 'howtoplay' && (
                      <div className="tab-panel">
                        <ul>
                          ...
                          <li>You receive 7 cards</li>
                          <li>Split into high hand (5 cards) and low hand (2 cards)</li>
                          <li>Both hands must beat dealer to win</li>
                        </ul>
                      </div>
                    )}
                    {rulesTab === 'rankings' && (
                      <div className="tab-panel">
                        <ul>
                          <li><strong>High Hand:</strong> Standard 5-card poker rankings</li>
                          <li><strong>Low Hand:</strong> Pair or high card</li>
                          <li>High hand must rank higher than low hand</li>
                          <li>Both hands compared to dealer</li>
                        </ul>
                      </div>
                    )}
                    {rulesTab === 'payouts' && (
                      <div className="tab-panel">
                        <ul>
                          <li>Win chance: 45%</li>
                          <li>Multipliers: 1x to 5x</li>
                          <li>Both hands must win for full payout</li>
                        </ul>
                      </div>
                    )}
                  </div>
