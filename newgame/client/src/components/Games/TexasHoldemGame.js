...
function TexasHoldemGame() {
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/games/texasholdem/play`,
        { bet },
      ...
      // Animate dealing player cards (first 2)
      const newCards = [];
      for (let i = 0; i < 2; i++) {
        setTimeout(() => {
      ...
      // Animate dealing community cards (next 5)
      for (let i = 2; i < 7; i++) {
        setTimeout(() => {
          newCards.push(allCards[i]);
          setCards([...newCards]);
          setAnimatingCards(prev => [...prev, `community-${i - 2}`]);
        }, (2 * 300) + ((i - 2) * 300));
      }
  ...
  return (
    <div className="game-container">
      ...
      <div className="texas-holdem-table">
        <div className="table-layout">
          <div className="game-column">
            <div className="table-felt">
              <div className="texas-holdem-area">
                {cards.length > 0 ? (
                  <div className="texas-holdem-cards">
                          ...
                          return (
                            <div key={index} className={`card ${isAnimating ? 'dealing' : ''} ${display.color}`}>
                              <div className="card-inner">
                                <div className={`card-corner card-top ${display.color}`}>
                                ...
                                <div className={`card-corner card-bottom ${display.color}`}>
                                  <div className="card-rank">{display.rank}</div>
                                  <div className="card-suit">{display.suit}</div>
                                </div>
                              </div>
                    ...
                    <div className="community-cards-section">
                      ...
                      <div className="cards-row">
                        {cards.slice(2, 7).map((card, index) => {
                          const display = getCardDisplay(card);
                          if (!display) return null;
                          const isAnimating = animatingCards.includes(`community-${index}`);
                          return (
                            <div key={index} className={`card ${isAnimating ? 'dealing' : ''} ${display.color}`}>
                              <div className="card-inner">
                                <div className={`card-corner card-top ${display.color}`}>
                  ...
                  <div className="game-placeholder">
                    <p>Place your bet and click "Play" to start</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="game-rules">
                ...
                <div className="rules-content">
                  <div className="rules-objective">
                    <p><strong>Objective:</strong> Make the best 5-card poker hand using your 2 cards and 5 community cards.</p>
                  </div>
