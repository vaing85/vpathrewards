...
export default function MerchantImportModal({ onClose, onDone }: Props) {
  ...
  const handlePreview = async () => {
    ...
    try {
      const { data } = await apiClient.post('/admin/merchants/preview', { csv: csvText });
  ...
  const handleImport = async () => {
    ...
    try {
      const { data } = await apiClient.post('/admin/merchants/import', { rows: readyRows });
      clearInterval(tick);
      setProgress(100);
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-2">
          <h2 className="text-xl font-bold text-gray-900">Import Merchants</h2>
          {step !== 'processing' && (
          ...
          {step === 'upload' && (
            <div className="flex-1 flex flex-col gap-5">
              ...
              <details className="text-xs text-gray-500 bg-gray-50 rounded-xl p-3">
                ...
                <p className="mt-2 font-mono">name, category, website_url, logo_url, cj_advertiser_id</p>
                <p className="mt-1">• First row must be the header</p>
                <p>• <code>name</code> is required; everything else is optional</p>
                <p>• <code>name</code> must be unique — existing names are skipped</p>
                <p>• <code>cj_advertiser_id</code> (numeric) links the merchant to CJ; the next CJ advertiser sync fills in its commission rate</p>
              </details>
          ...
          {step === 'preview' && (
            <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">CJ ID</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {preview.map((row) => (
                      <tr key={row.row} className={row.status === 'error' ? 'bg-red-50' : row.status === 'duplicate' ? 'bg-yellow-50' : ''}>
                        ...
                        <td className="px-4 py-2.5 font-medium text-gray-700">{row.name || <span className="text-red-400 italic">missing</span>}</td>
                        <td className="px-4 py-2.5 text-gray-600 max-w-[160px] truncate">{row.category || <span className="text-gray-300">—</span>}</td>
                        <td className="px-4 py-2.5 text-gray-600 font-mono text-xs">{row.cj_advertiser_id || <span className="text-gray-300">—</span>}</td>
              ...
              <div className="flex gap-3">
                ...
                >
                  Import {readyRows.length} merchant{readyRows.length !== 1 ? 's' : ''} →
                </button>
              </div>
            </div>
          )}
                ...
                <div className="flex-1 overflow-auto rounded-xl border border-gray-200">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {results.map((r) => (
                        <tr key={r.row}>
                          <td className="px-4 py-2.5 text-gray-700 max-w-[260px] truncate">{r.name}</td>
                          <td className="px-4 py-2.5">
