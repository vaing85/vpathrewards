...
router.post('/track', authenticateToken, validateCashbackTrack, async (req: AuthRequest, res) => {
  try {
    ...
    // Calculate cashback
    const cashbackAmount = (amount * offer.cashback_rate) / 100;

    // Create transaction
    const result = await dbRun(
      'INSERT INTO cashback_transactions (user_id, offer_id, amount, status) VALUES (?, ?, ?, ?)',
      [req.userId, offer_id, cashbackAmount, 'pending']
    );
    ...
    await dbRun(
      ...
      [cashbackAmount, req.userId]
    );

    // Create referral earning if user was referred (async)
    createReferralEarning(req.userId, transactionId, cashbackAmount).catch(err => {
