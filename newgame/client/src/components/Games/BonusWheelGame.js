...
import './BonusWheelGame.css';

function BonusWheelGame() {
  const { user, fetchUser } = useAuth();
  const { showError, showInsufficientBalance } = useNotification();
  const navigate = useNavigate();
  const [settings, updateSettings] = useGameSettings('bonuswheel', {
    bet: 10
  });
  ...
  const wheelSegments = [
    { multiplier: 3, color: '#ff6b6b', label: '×3' },
    { multiplier: 5, color: '#4ecdc4', label: '×5' },
    { multiplier: 10, color: '#ffe66d', label: '×10' },
    { multiplier: 25, color: '#a8e6cf', label: '×25' },
    { multiplier: 50, color: '#ffd93d', label: '×50' },
  ...
  return (
    <div className="bonuswheel-game-container">
      <GameHeader 
        title="🎁 Bonus Wheel"
        balance={balance}
        backPath="/dashboard"
      ...
      <div className="bonuswheel-main">
        <div className="bonuswheel-game-layout">
          <div className="bonuswheel-board">
          ...
          <div className="bonuswheel-controls">
            <div className="bet-control">
              ...
              <div className="bet-amount-controls">
                <button
                  ...
                  disabled={bet <= 5 || spinning || result}
                >
                  −
                </button>
                <input
                ...
                <button
                  ...
                  disabled={bet >= Math.min(user?.balance || 0, 100) || spinning || result}
                >
                  +
                </button>
              </div>
